...
load exchcd
load tcosts
load Adjusted_Forecast_error_CI





%使用res时候，IBES的公司数量和CRSP的公司数量不一致，重新跑setup从1990年开始，1970年开始是CRSP是648个月份，而IBES1990-02才开始
%可以试试取绝对值abs{Adjusted_Forecast_error_CI}，如果偏差越大实际上代表，市场实际表现显著偏离了预测值，也代表不确定性越大，然后根据偏差大的可能有较大风险，带来较大的收益，所以做多偏差大的，做空偏差小的

% 使用 makeUnivSortInd 函数
ind = makeUnivSortInd(Adjusted_Forecast_error_CI, 5); % 这里假设你要分成5个投资组合
res1 = runUnivSort(ret, ind, dates, me);  
%尝试使用equal weright 之前是value-weight方法
...
ind = makeUnivSortInd(Adjusted_Forecast_error_CI, [30 70]);                                            
res_FE_equal_30_70 = runUnivSort(ret, ind, dates, me, 'weighting', 'e');   
%尝试FF6因子模型作为参照
% 6-factor model alphas and loadings
ind = makeUnivSortInd(Adjusted_Forecast_error_CI, 5); % 这里假设你要分成5个投资组合
res_FF6 = runUnivSort(ret, ind, dates, me, 'factorModel', 6);  

%尝试分为10组呢，equal weight FF4
ind = makeUnivSortInd(Adjusted_Forecast_error_CI, 10); % 这里假设你要分成10个投资组合
res_equal_10 = runUnivSort(ret, ind, dates, me,'weighting', 'e');  

%尝试分为10组呢 ，equal weight FF4，
% 2-month holding period
res9 = runUnivSort(ret, ind, dates, me, 'holdingPeriod', 12,'weighting', 'e');   

%尝试分为10组呢 ，equal weight FF6，
% 6-factor model alphas and loadings
res10 = runUnivSort(ret, ind, dates, me, 'factorModel', 6,'holdingPeriod', 6,'weighting', 'e');  


%尝试分为10组 ，equal weight FF6，holding 12 ,trading cost
% Calculate tcosts, net returns, and turnover usging the tcosts matrix
ind = makeUnivSortInd(Adjusted_Forecast_error_CI, 10); % 这里假设你要分成10个投资组合
res17 = runUnivSort(ret, ind, dates, me, 'tcosts', tcosts,'holdingPeriod', 12,'weighting', 'e');         



%  makeUnivSortInd 5vportfilio
ind = makeUnivSortInd(Adjusted_Forecast_error_CI, 5); % 这里假设你要分成5个投资组合
res1 = runUnivSort(ret, ind, dates, me);  
